...
from erpnext.stock.report.stock_projected_qty.stock_projected_qty import (
    execute as execute_proj_qty,
)
from erpnext_crystal_ball.erpnext_crystal_ball.report.expected_sales_forcast.expected_sales_forcast import (
    execute as execute_exp_forcast,
)
from erpnext.manufacturing.report.bom_stock_calculated.bom_stock_calculated import (
    execute as get_raw_matrial,
)
import datetime
...
class AdjustedStockQty:
    ...
    def __init__(self, filters):
        ...
        self.months = self.get_months_range(
        ...
        self.raw_item_list = frappe.db.get_list(
            "Item", fields=["name"], filters={"item_group": ["=", "RAW"]}, pluck="name"
        )

        self.pack_item_list = frappe.db.get_list(
            "Item", fields=["name"], filters={"item_group": ["=", "PACK"]}, pluck="name"
        )
    ...
    def get_forcast_data(self, month, fg_forcast):
        ...
        for fg_item in fg_forcast:
            items_dicts = []
            item_code = fg_item["code"]
            default_bom = frappe.db.get_value("Item", item_code, "default_bom")
            quantity = fg_item.get(month)
        ...
        for raw_material_code, raw_material_info in self.data_structure.items():
            ...
            if raw_material_code in self.material_list:
                ...
                processed_item = {
                    ...
                    "avail_qty": raw_material_info.get("avil_qty"),
                ...
                previous_diff_qty = raw_material_info.get("actual_qty")

                self.set_calculated_material_qty(
                    raw_material_info, processed_item, previous_diff_qty
                )

    def set_calculated_material_qty(
        self, raw_material_info, processed_item, previous_diff_qty
    ):
    ...
    def filter_by_material_type(self):
        ...
        if self.group == "raw material":
            ...
            self.material_list = (
                self.raw_list + self.pack_list
            )  # Show all if no specific group selected
    ...
    def processes_raw_material(self, item_code, month, quantity, items_dicts):
        ...
        for item in items_dicts:
            ...
            raw_required_qty = quantity * item.get("qty_per_unit")

            ordered_qty, reserved_qty = self.get_projected_stock(raw_material_code)

            safety_stock, lead_time, coverage_days = frappe.db.get_value(
                "Item",
                raw_material_code,
                ["safety_stock", "lead_time_days", "custom_coverage_days"],
            )

            actual_safety_qty = self.calculate_actual_safety_quantity(
                safety_stock, avil_qty, ordered_qty, reserved_qty
            )
            ...
            if raw_material_code not in self.data_structure:
                self.data_structure[raw_material_code] = {
                    ...
                    f"{month}_diff_qty": 0,
                    "FGs": {fg_item_code: {f"{month}_req_qty": raw_required_qty}},
                    "total_months_req": {f"total_{month}_req_qty": raw_required_qty},
                }

            else:

                if (
                    f"total_{month}_req_qty"
                    not in self.data_structure[raw_material_code]["total_months_req"]
                ):
                    self.data_structure[raw_material_code]["total_months_req"][
                        f"total_{month}_req_qty"
                    ] = 0
    ...
    def classify_raw_material(self, raw_material_code):
        ...
        if (
            raw_material_code in self.raw_item_list
            and raw_material_code not in self.raw_list
        ):
            self.raw_list.append(raw_material_code)

        elif (
            raw_material_code in self.pack_item_list
            and raw_material_code not in self.pack_list
        ):
    ...
    def get_projected_stock(self, raw_material_code):
        ...
        projected_data = frappe.get_list(
            "Bin",
            filters={"item_code": raw_material_code},
    ...
    def calculate_actual_safety_quantity(
        self, safety_stock, avil_qty, ordered_qty, reserved_qty
    ):
