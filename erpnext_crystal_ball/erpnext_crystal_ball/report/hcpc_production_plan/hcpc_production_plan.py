...
def get_columns(filters=None):
	columns= [
		    {"fieldname": "products", "label": _("Products"), "fieldtype": "Link", "options": "Item", "width": 150,"sticky":True},
			{"fieldname": "workstation_line", "label": _("Workstation Line"), "fieldtype": "Link",  "options":"Workstation", "width": 150},

			{"fieldname": "total_hours", "label": _("Total Hours"), "fieldtype": "Float", "width": 120},
			{"fieldname": "remaining_hours", "label": _("Remaining Hours"), "fieldtype": "Float", "width": 120},
			{"fieldname": "target_sales", "label": _("Target Sales"), "fieldtype": "Float", "width": 120},
			{"fieldname": "promotion_sales", "label": _("Promotion Sales"), "fieldtype": "Float", "width": 120},
			{"fieldname": "curr_stock", "label": _("Current Stock"), "fieldtype": "Float", "width": 120},

			{"fieldname": "speed", "label": _("Speed"), "fieldtype": "Int", "width": 120},
			{"fieldname": "hrs_done", "label": _("Hours Done"), "fieldtype": "Float", "width": 120},
			{"fieldname": "local_hrs", "label": _("Local Hours"), "fieldtype": "Float", "width": 120},
			{"fieldname": "export_hrs", "label": _("Export Hours"), "fieldtype": "Float", "width": 120},
		]
	...
	columns.append({"fieldname": "total", "label": _("Total"), "fieldtype": "Float", "width": 120}),
...
def get_data():
	...
	for item in item_list:
		...
		workstation_line = get_workstation_from_bom(default_bom) if default_bom else "No BOM"
		speed=get_bom_speed(default_bom)
		
		hrs_done=fg_stock_qty / speed if speed else 0

		weekly_sales = {f"week_{i}": expected_sales_dict.get(item_code, {}).get(f"week_{i}", 0) for i in range(1, 6)}
		...
		promotion_sales = expected_sales_dict.get(item_code, {}).get("promotion_sales",0)
...
def get_expected_sales():
	...
	expected_sales_docs=frappe.db.get_list("Expected Sales",fields=['name','expected_date','expected_end'],
	...
	for doc in expected_sales_docs:
		...
		for item in item_records:
			...
			if item_code not in item_sales_by_week:
				item_sales_by_week[item_code] = {
					f"week_{i}": 0 for i in range(1, 6)
				}
				item_sales_by_week[item_code]["target_sales"] = 0
				item_sales_by_week[item_code]["promotion_sales"] = 0

			item_sales_by_week[item_code][f"week_{week_num}"] += item_qty

			if is_promotion:
				item_sales_by_week[item_code]["promotion_sales"] += item_qty
			else:
				item_sales_by_week[item_code]["target_sales"] += item_qty
...
def get_workstation_from_bom(bom_name):
    ...
    workstation = frappe.db.get_value("BOM Operation", {"parent": bom_name}, "workstation")
